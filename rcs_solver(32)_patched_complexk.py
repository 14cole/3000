...
def _hankel2_0_array(x: np.ndarray) -> np.ndarray:
    ...
    if _SCIPY_SPECIAL is not None:
        try:
            return np.asarray(_SCIPY_SPECIAL.hankel2(0, z), dtype=np.complex128)
        except Exception:
            pass
    vec = np.vectorize(_hankel2_0, otypes=[np.complex128])
...
def _hankel2_1_array(x: np.ndarray) -> np.ndarray:
    ...
    if _SCIPY_SPECIAL is not None:
        try:
            return np.asarray(_SCIPY_SPECIAL.hankel2(1, z), dtype=np.complex128)
        except Exception:
            pass
    vec = np.vectorize(_hankel2_1, otypes=[np.complex128])
    return np.asarray(vec(z), dtype=np.complex128)
...
def _backscatter_rcs_coupled_many_linear(
    ...
    sigma_lin = _rcs_sigma_from_amp(amp, k_air, normalization_mode=normalization_mode)
    ...
    if np.any(far_mask) and _SCIPY_SPECIAL is not None:
        ...
        for m0 in range(0, n, BATCH):
            ...
            if _k0_is_real:
                ...
                # Lossy media: complex k.
                # Direct scipy.special.hankel2(...) can overflow internally for large
                # complex arguments even when the final H^(2) value is finite/small.
                # Use the exponentially scaled form first and reconstruct H^(2) with
                # exp(-j z); for the passive-media branch here Im(z) <= 0, so this
                # reconstruction is decaying rather than exploding.
                kr = (complex(k0) * dist).ravel()
                if hasattr(_SCIPY_SPECIAL, "hankel2e"):
                    with np.errstate(over='ignore', under='ignore', invalid='ignore', divide='ignore'):
                        h0 = np.asarray(_SCIPY_SPECIAL.hankel2e(0, kr) * np.exp(-1j * kr), dtype=np.complex128)
                        h1 = np.asarray(_SCIPY_SPECIAL.hankel2e(1, kr) * np.exp(-1j * kr), dtype=np.complex128)
                else:
                    with np.errstate(over='ignore', under='ignore', invalid='ignore', divide='ignore'):
                        h0 = np.asarray(_SCIPY_SPECIAL.hankel2(0, kr), dtype=np.complex128)
                        h1 = np.asarray(_SCIPY_SPECIAL.hankel2(1, kr), dtype=np.complex128)

                if (not np.all(np.isfinite(h0))) or (not np.all(np.isfinite(h1))):
                    # Rare fallback for any remaining bad entries: use the existing
                    # scalar complex Hankel helpers, which already route through
                    # SciPy/mpmath/native series-asymptotic logic.
                    bad0 = ~np.isfinite(h0)
                    bad1 = ~np.isfinite(h1)
                    if np.any(bad0):
                        idxs = np.flatnonzero(bad0)
                        for idx_bad in idxs:
                            h0[idx_bad] = _hankel2_0(complex(kr[idx_bad]))
                    if np.any(bad1):
                        idxs = np.flatnonzero(bad1)
                        for idx_bad in idxs:
                            h1[idx_bad] = _hankel2_1(complex(kr[idx_bad]))

                h0 = h0.reshape(B, n, Q_FAR)
                h1 = h1.reshape(B, n, Q_FAR)
...
def _normalize_rcs_normalization_mode(mode: str | None) -> str:
    ...
    if text in {"no_k", "without_k", "nok", "legacy", "historical", "production"}:
        return RCS_NORM_MODE_NO_K
    raise ValueError(
        f"Unsupported rcs_normalization_mode '{mode}'. "
        "Supported modes are 'physical' and 'no_k'."
    )
...
def _rcs_sigma_from_amp(
    ...
    """Apply the requested 2D scattering-width normalization to the far-field amplitude."""

    amp_eval = np.asarray(amp_vec, dtype=np.complex128)
    mode = _normalize_rcs_normalization_mode(normalization_mode)
    if mode == RCS_NORM_MODE_PHYSICAL:
...
def solve_monostatic_rcs_2d(
    ...
    - rcs_normalization_mode controls post-processing normalization; the default
      physical mode stores sigma_2d = |A|^2 / (4 k), while 'no_k' remains
      available only for legacy comparison workflows.
        ...
        if coupled_mode:
            if linear_galerkin_mode:
                ...
                rcs_lin_vec, amp_vec = _backscatter_rcs_coupled_many_linear(
                    mesh=mesh,
                    infos=coupled_infos,
                    u_trace_nodes_mat=sol_mat[:nnodes, :],
                    q_minus_nodes_mat=sol_mat[nnodes:, :],
                    k_air=k0,
                    elevations_deg=elevations_arr,
    ...
    result = {
        ...
        "metadata": {
            ...
            "rcs_normalization_formula": (f"sigma = ({RCS_NORM_NUMERATOR:.12g} / k) * |A|^2" if rcs_norm_mode == RCS_NORM_MODE_PHYSICAL else f"sigma = ({RCS_NORM_NUMERATOR:.12g}) * |A|^2"),
